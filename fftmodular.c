...
{   
    ...
    for (int i = 0; i < iters; i += 1) {
        ...
        out_re = out_re - 3*write_offset + 4;
        SIMD_ADD(R7, Dr, Bi); // F3 im
        _mm256_store_pd(out_im, R7);
        out_im = out_im - 3*write_offset + 4;
    }
}
                 ...
                 int read_offset, int write_offset, int stage) 
{
    ...
    for (int i = 0; i < iters; i += 1) {
        ...
        if (stage == 0) {
            ...
            twiddle_r += read_offset;
            R10 = _mm256_load_pd(twiddle_i);
            twiddle_i += read_offset;
        } else {
            R9 = _mm256_broadcast_sd(twiddle_r);
            twiddle_r += iters;
            R10 = _mm256_broadcast_sd(twiddle_i);
            twiddle_i += iters;
        }
        ...
        if (stage == 0) {
            ...
            R10 = _mm256_broadcast_sd(twiddle_r);
            twiddle_r += iters;
            R11 = _mm256_broadcast_sd(twiddle_i);
            twiddle_i += iters;
        }
        ...
        if (stage == 0) {
            ...
            R12 = _mm256_broadcast_sd(twiddle_r);
            twiddle_r += iters;
            R13 = _mm256_broadcast_sd(twiddle_i);
            twiddle_i += iters;
        }
        ...
        if (stage == 0) {
            ...
            twiddle_r -= read_offset*3;
            twiddle_r += 4;
            R14 = _mm256_load_pd(twiddle_i);
            twiddle_i -= read_offset*3;
            twiddle_i += 4;   
        } else {
            R13 = _mm256_broadcast_sd(twiddle_r);
            twiddle_r -= 3*iters;
            twiddle_r += 1;
            R14 = _mm256_broadcast_sd(twiddle_i);
            twiddle_i -= 3*iters;
            twiddle_i += 1;
        }
...
int main() {
    ...
    double twiddle_r[64], twiddle_i[64];
    double pi = 3.14159265358979323846;
    for(int k=0; k<64; k++) {
         double theta = -2.0 * pi * k / 64.0;
         twiddle_r[k] = cos(theta);
         twiddle_i[k] = sin(theta);
    }
    ...
    for(int i=0; i<RUNS; i++) {
        ...
        for(int j = 0; j < iters; j++) {
            ...
            fft4_kernel_transpose(i_re, i_im, o_re, o_im, 
                n_doubles, twiddle_r, twiddle_i, n_doubles/4, 4, j);
            temp = i_re;
            i_re = o_re;
            o_re = temp;
            temp = i_im;
        ...
        fft4_kernel(i_re, i_im, out_re, out_im, n_doubles, n_doubles/4, n_doubles/4);
    }
